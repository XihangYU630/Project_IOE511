...
% end
% fprintf(fileID, '%s %d %f %f % f\r\n', strcat(method.name, method.options.step_type), k, elapsedTime, f_record(end), norm_g);
% fclose(fileID);


fileName = strcat(method.name, method.options.step_type, '.txt');
fileID = fopen(fileName, 'a');
if fileID == -1
    error('File could not be opened');
end
fprintf(fileID, '%s %d %f %f %f\r\n', problem.name, k, elapsedTime, f_record(end), norm_g);
fclose(fileID);
